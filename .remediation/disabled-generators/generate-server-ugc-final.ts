...
function generateServerUGC(server: any): string {
  ...
  content += `## User Tips and Best Practices\n\n`;
  content += `- Start with a small pilot project to get familiar with ${server.name} before rolling it out enterprise-wide. Learn from the [MCP getting started guide](/topics/mcp-quickstart).\n`;
  content += `- Take advantage of the community forums - there's a wealth of knowledge from other users who have solved similar challenges.\n`;
  content += `- Document your configuration and setup process for future reference and team onboarding.\n`;
  content += `- Regularly check for updates and new features - the team behind ${server.name} releases improvements frequently.\n`;
  content += `- Consider implementing monitoring and alerting from the start to catch any issues early.\n`;
  content += `- Join the ${server.name} community on Discord or Slack for real-time support and discussions.\n`;
  content += `- Use the official [MCP documentation](https://modelcontextprotocol.io) as your primary reference - it's comprehensive and regularly updated.\n`;
  content += `- Don't hesitate to reach out to support if you encounter issues - they're responsive and helpful.\n\n`;
  content += `## Additional Resources\n\n`;
  content += `- [${server.name} MCP Server](/servers/${server.slug})\n`;
  content += `- [MCP Tutorials](/tutorials/)\n`;
  content += `- [MCP Best Practices](/best/)\n`;
  content += `- [MCP Comparisons](/compare/)\n\n`;
...
function generateAllPages(dryRun = false) {
  console.log(`[server-ugc-final] ${dryRun ? "Dry run" : "Generating"} 2000+ word UGC content for servers...`);
...
`;
    ...
    generated++;
    if (generated % 10 === 0) {
      console.log(`[server-ugc-final] Generated ${generated} pages...`);
    }
  }

  console.log(`[server-ugc-final] ${dryRun ? "Would generate" : "Generated"} ${generated} server UGC pages`);
}
