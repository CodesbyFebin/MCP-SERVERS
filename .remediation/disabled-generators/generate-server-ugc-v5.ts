...
function generateServerUGC(server: any): string {
  ...
  content += `- Start with a small pilot project to get familiar with ${server.name} before rolling it out enterprise-wide.\n`;
  ...
  content += `- Use the official documentation as your primary reference - it's comprehensive and regularly updated.\n`;
...
function generateAllPages(dryRun = false) {
  console.log(`[server-ugc-v5] ${dryRun ? "Dry run" : "Generating"} 2000+ word UGC content for servers...`);
...
`;
    ...
    generated++;
    if (generated % 10 === 0) {
      console.log(`[server-ugc-v5] Generated ${generated} pages...`);
    }
  }

  console.log(`[server-ugc-v5] ${dryRun ? "Would generate" : "Generated"} ${generated} server UGC pages`);
}
