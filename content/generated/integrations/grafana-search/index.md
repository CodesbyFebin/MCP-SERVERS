---
title: "Grafana Search MCP Integration Guide - Complete Grafana Search MCP Guide"
description: "Complete MCP server guide for Grafana Search. Learn implementation, configuration, security, and best practices for Grafana Search MCP integration with code examples and tutorials."
canonical: "https://www.mcpserver.in/integrations/grafana-search/"
h1: "Grafana Search MCP Integration Guide"
status: "candidate"
...
primary_entity: "Grafana Search"
primary_keyword: "Grafana Search MCP integration"
...
These capabilities are not theoretical. In the sections that follow we demonstrate each one with concrete configuration and code, and we explain the trade-offs you should weigh when designing Grafana Search into a production system.
...
- **Networking**: Outbound HTTPS (port 443) access to the Grafana Search MCP integration endpoint and, for remote mode, inbound access to the MCP port.
- **Credentials**: A service account, API token, or OAuth client configured with the minimum scopes Grafana Search needs.

### Installation Methods

You can integrate Grafana Search using whichever method matches your operational model.

#### Method 1: Direct Installation

```bash
npm install @modelcontextprotocol/server-grafana-search
npx mcp-server-grafana-search --help
```

Direct installation is ideal for local development and for embedding Grafana Search inside another Node.js application.

#### Method 2: Docker Container

```bash
docker pull mcpserver/grafana-search:latest
docker run -p 3000:3000 -e MCP_AUTH_TOKEN=your-token mcpserver/grafana-search:latest
```
...
```bash
kubectl apply -f https://raw.githubusercontent.com/mcpserver/grafana-search/main/k8s/deployment.yaml
```
...
Whichever method you choose, the next sections show how to configure Grafana Search so it is secure by default and ready to connect to your Grafana Search MCP integration environment.
...
Grafana Search enforces authentication on every connection and authorization on every tool call:
...
kubectl create secret generic mcp-grafana-search-secrets \
...
Validation verifies that Grafana Search is healthy, discoverable, and correctly executing tools before you route real agent traffic to it. Run these checks in order; each depends on the previous one succeeding.
...
Attempt a mutating tool with a read-only token and confirm it is rejected with a `403` rather than executed. This proves your RBAC policy is active. Only when all five steps pass should you promote Grafana Search to production traffic.
...
- These figures are environment-dependent; load-test your own Grafana Search deployment before relying on them.
...
### Q: Is Grafana Search open source?

A: Grafana Search follows an open-core model. The Model Context Protocol specification is open source under a permissive license, and the reference server is freely available. Advanced enterprise features such as federated auth and multi-region caching may require a commercial edition.
...
### Q: Can I run Grafana Search in a Docker container?

A: Yes. Grafana Search publishes official container images and a Helm chart for Kubernetes. The deployment guide covers both in detail.

### Q: How is Grafana Search different from a traditional REST API?
...
### Q: How do I scale Grafana Search for production?

A: Deploy behind a load balancer with horizontal pod autoscaling keyed on CPU and request latency. Shared nothing design means you can run many replicas behind one external auth layer. Cache read-heavy tools and set per-client rate limits to protect downstream Grafana Search MCP integration systems.

## Grafana Search: A Domain-Specific Deep Dive


Grafana Search is catalogued under the **observability** cluster and the **grafana-search** sub-cluster, which means its design assumptions differ from a generic MCP server. Its primary job is to make Grafana Search MCP integration available to agents through a governed, discoverable surface rather than an ad-hoc script.

### How Grafana Search compares to its siblings
...
### Integration shape for Grafana Search

The typical call path through Grafana Search is: the agent opens a transport session, completes the `initialize` handshake, calls `tools/list` to learn what Grafana Search can do, and then invokes individual tools with validated JSON. Because Grafana Search declares its own capability set, the agent never hard-codes endpoints for Grafana Search MCP integration. This is what lets Grafana Search be swapped for GitHub behind a gateway without rewriting the agent.

### Cluster-specific guidance (observability)

For the observability domain, prioritize least-privilege scopes for Grafana Search, cache read-heavy Grafana Search MCP integration tools, and monitor per-tool latency so regressions are caught before users notice.

### Composing Grafana Search with GitHub

Rather than a single monolithic server, production deployments in the observability domain usually compose Grafana Search with GitHub and other siblings behind one MCP gateway. The gateway owns auth, rate limiting, and audit logging; each server owns its own tool surface. This separation keeps blast radius small and lets you promote or roll back Grafana Search independently.
...
<script type="application/ld+json">
{
  ...
  "@type": "TechArticle",
  "name": "Grafana Search MCP Integration Guide",
  "description": "Complete MCP server guide for Grafana Search. Learn implementation, configuration, and best practices.",
  "url": "https://www.mcpserver.in/integrations/grafana-search/",
  "inLanguage": "en-US",
  "isPartOf": {
    "@type": "WebSite",
    "name": "MCPserver.in",
  ...
  "breadcrumb": {
    ...
    "itemListElement": [
      ...
      },
      {
        ...
        "position": 4,
        "name": "Grafana Search",
        "item": "https://www.mcpserver.in/integrations/grafana-search/"
      }
    ]
  },
