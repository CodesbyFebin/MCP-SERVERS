...
import {
  ...
  publishedServers as servers,
  routeCandidateBacklog,
...
function add(path: string, type: string, title: string, qualityScore = null, contentHash = null) {
  rows.push({
    route: path,
    contentType: type,
    status: "published",
...
servers.forEach((s: any) => {
  const profile = getServerPublicationProfile(s.slug);
  add(
    `/servers/${s.slug}/`,
    "server",
    s.name || s.slug,
    profile?.quality.total ?? null,
    profile?.contract.contentHash ?? null,
  );
  const latest = rows[rows.length - 1];
  latest.sitemap = "sitemap-integrations.xml";
  latest.evidenceCount = profile?.evidence.length ?? null;
  latest.claimCount = profile?.claims.length ?? null;
});
...
const csvRows = ["route,contentType,status,canonical,indexability,sitemap,title,primaryIntentId,wordCount,qualityScore,contentHash,evidenceCount,claimCount,updatedAt"];
for (const r of rows) {
  csvRows.push(
    [
      r.route,
...
fs.writeFileSync(
  path.join(reportsDir, "candidate-route-backlog.json"),
  JSON.stringify(
    routeCandidateBacklog.map((contract) => ({
      ...
      reason: "Candidate inventory only. Requires evidence, intent validation, content contract, QA, and editorial approval before publication.",
    })),
    null,
    2,
  ),
...
const summary = `# URL Inventory Summary\n\nGenerated: ${new Date().toISOString()}\nPublished routes: ${rows.length}\nCandidate route patterns held back: ${routeCandidateBacklog.length}\n\n` + rows.reduce(
  (acc, r) => {
