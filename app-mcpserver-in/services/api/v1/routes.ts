...
api.post("/execution", async (c) => {
  ...
  const { serverId, toolId, args } = body;
  ...
  // Execute the tool via FastMCP backend
  // In production: would call services/mcp-server via JSON-RPC
  try {
    ...
    return json({
      ...
      // Would return streaming JSON-RPC response in production
    });
  } catch (err) {
